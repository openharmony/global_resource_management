...
const std::string SYSTEM_RESOURCE_PATH_COMPRESSED = "/data/global/systemResources/SystemResources.hap";

const std::string SYSTEM_RESOURCE_OVERLAY_PATH_COMPRESSED = "/sys_prod/app/" \
    "SystemResourcesOverlay/SystemResourcesOverlay.hap";

const std::string SYSTEM_RESOURCE_NO_SAND_BOX_PKG_PATH = "/system/app/ohos.global.systemres/SystemResources.hap";

const std::string SYSTEM_RESOURCE_EXT_NO_SAND_BOX_PKG_PATH = "/system/app/ohos.global.systemres/SystemResourcesExt.hap";

const std::string SYSTEM_RESOURCE_NO_SAND_BOX_HAP_PATH = "/system/app/SystemResources/SystemResources.hap";

const std::string SYSTEM_RESOURCE_EXT_NO_SAND_BOX_HAP_PATH = "/system/app/SystemResources/SystemResourcesExt.hap";
...
{
    std::string sysPkgNamePath = isSandbox ? SYSTEM_RESOURCE_PATH : SYSTEM_RESOURCE_NO_SAND_BOX_PKG_PATH;
    std::string sysHapNamePath = isSandbox ? SYSTEM_RESOURCE_PATH_COMPRESSED : SYSTEM_RESOURCE_NO_SAND_BOX_HAP_PATH;
    auto loadResources = [&impl](const std::string &basePath, const std::string &extPath) -> bool {
        bool result = false;
        if (Utils::IsFileExist(SYSTEM_RESOURCE_OVERLAY_PATH_COMPRESSED)) {
            result = impl->AddResource(basePath, {SYSTEM_RESOURCE_OVERLAY_PATH_COMPRESSED});
        } else {
            result = impl->AddResource(basePath.c_str());
        }
        if (result && Utils::IsFileExist(extPath)) {
            result = impl->AddResource(extPath.c_str());
        }
        return result;
    };

    if (Utils::IsFileExist(sysPkgNamePath)) {
        return loadResources(sysPkgNamePath, SYSTEM_RESOURCE_EXT_NO_SAND_BOX_PKG_PATH);
    }

    if (Utils::IsFileExist(sysHapNamePath)) {
        return loadResources(sysHapNamePath, SYSTEM_RESOURCE_EXT_NO_SAND_BOX_HAP_PATH);
    }
    return false;
}
